...
export function genSummoningCircles(w,h,amt,zoom){
  ...
  for(let i=0;i<count;i++){
    ...
    const seal = pick(['glyph','glyph','star','dot']);
...
export function genBlackHole(w,h,amt,zoom){
  ...
  for(let i=0;i<nField;i++){
    ...
    ctx.save(); ctx.translate(cx+dx/b*b1, cy+dy/b*b1); ctx.rotate(Math.atan2(dy,dx)+Math.PI/2);
    ctx.globalAlpha=a; ctx.fillStyle=`rgb(${tone},${tone},${tone})`;
    ctx.beginPath(); ctx.ellipse(0,0,size*Math.min(3,stretch),size,0,0,Math.PI*2); ctx.fill(); ctx.restore();
  ...
  const orbits=Math.round(90*Math.min(2,0.5+amt*0.6));
  const orbitsR=[];
