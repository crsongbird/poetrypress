...
export function genFlowers(w,h,amt,zoom){
  ...
  // The pond: a few very soft, very wide bands of light lying flat across the
  // ground. Not waves, not ripples — just the sense of a still surface.
  for(let i=0;i<4;i++){
    const y=h*(0.1+Math.random()*0.8), band=h*(0.08+Math.random()*0.14);
    const g=ctx.createLinearGradient(0,y-band,0,y+band);
    g.addColorStop(0,'rgba(236,236,236,0)'); g.addColorStop(0.5,'rgba(236,236,236,0.07)');
    g.addColorStop(1,'rgba(236,236,236,0)');
    ctx.fillStyle=g; ctx.fillRect(0,y-band,w,band*2);
  }

  // Size is the LARGEST bloom; each is 10–100% of it. Blooms are placed so they
  // never touch: a candidate that would overlap is thrown away, so raising the
  // size spreads them out, and at high counts the small ones fill the gaps.
  const maxR = unit*0.085*zoom;
  const target = Math.max(2, Math.round((w*h)/285000 * amt));
  const placed = [];
  // Collision is CIRCULAR (centre distance against the sum of radii), and
  // tight: blooms may brush petal tips (0.82 of each radius counts), so they
  // crowd the way lilies do. Sizes run 10–100% of the largest; placing the
  // largest first lets the small ones fall into the gaps they leave.
  const radii = [];
  for(let i=0;i<target;i++) radii.push(maxR*(0.1 + Math.pow(Math.random(), 1.6)*0.9));
  radii.sort((a,b)=>b-a);
  ...
  const petal=(cx,cy,a,len,wide,round)=>{
    ...
    // round > 0 carries the flanks further toward the tip: a blunter, rounder petal
    ...
    ctx.bezierCurveTo(x1,y1,x2,y2,tx,ty); ctx.bezierCurveTo(x3,y3,x4,y4,cx,cy); ctx.closePath();
  };

  for(const {x:cx, y:cy, r:R} of placed){
    const rot=Math.random()*Math.PI*2;
    const dark=Math.random()<0.5, tone=dark?54:220, ink=`rgb(${tone},${tone},${tone})`;
    const alpha=0.5+Math.random()*0.3;
    // its shadow on the water, soft and slightly offset
    const sh=ctx.createRadialGradient(cx+R*0.12,cy+R*0.16,R*0.2,cx+R*0.12,cy+R*0.16,R*1.25);
    sh.addColorStop(0,'rgba(24,24,24,0.16)'); sh.addColorStop(1,'rgba(24,24,24,0)');
    ctx.globalAlpha=1; ctx.fillStyle=sh; ctx.beginPath(); ctx.arc(cx+R*0.12,cy+R*0.16,R*1.25,0,Math.PI*2); ctx.fill();

    ctx.strokeStyle=ink; ctx.fillStyle=ink; ctx.lineWidth=Math.max(0.5,R*0.018);
    // outer petals broad with a softly pointed tip — narrow ones made a star;
    // the inner rings rounder still
    const rings=[{n:11,len:1.00,wide:0.44,a:alpha*0.42,off:0.0,round:0.55},
                 {n:9, len:0.70,wide:0.36,a:alpha*0.60,off:0.5,round:1},
                 {n:7, len:0.46,wide:0.34,a:alpha*0.78,off:0.25,round:1.4}];
    for(const g of rings){
      for(let k=0;k<g.n;k++){
        petal(cx,cy,rot+((k+g.off)/g.n)*Math.PI*2,R*g.len,R*g.wide,g.round);
        ctx.globalAlpha=g.a; ctx.fill();
        ctx.globalAlpha=Math.min(1,g.a*1.5); ctx.stroke();
      }
    }
    ctx.globalAlpha=alpha*0.85; ctx.lineWidth=Math.max(0.35,R*0.010);
    ...
    ctx.globalAlpha=alpha; ctx.beginPath(); ctx.arc(cx,cy,R*0.145,0,Math.PI*2); ctx.fill();
    const hole=dark?205:46; ctx.fillStyle=`rgb(${hole},${hole},${hole})`;
    const holes=8+Math.floor(Math.random()*5);
    for(let k=0;k<holes;k++){
      const a=(k/holes)*Math.PI*2+rot, rr=R*(0.04+Math.random()*0.065);
      ctx.beginPath(); ctx.arc(cx+Math.cos(a)*rr,cy+Math.sin(a)*rr,Math.max(0.4,R*0.017),0,Math.PI*2); ctx.fill();
    }
  }
  ctx.globalAlpha=1;
  return c;
